...
let barChart = "";   // Move this outside the loop so all rows accumulate

for (let i = 0; i < phones.length; i++) {  // Fixed counter: start at 0 and use < phones.length
   
   // Calculate the percent of total sales for a particular phone model
   let barPercent = Math.round(100 * sales[i] / totalSales);  // Fixed: use sales[i] and round
   ...
   switch (phones[i]) {   // Fixed syntax: added { } and colons
   ...
   // Table row containing information on phone and total sales (formatted using a thousands-separator)
   ...
   for (let j = 0; j < barPercent; j++) {   // Fixed: start at 0 and use < barPercent
      barChart += cellTag + "</td>";
   }
   barChart += "</tr>";
}
...
document.getElementsByTagName("tbody")[0].insertAdjacentHTML("beforeEnd", barChart);  // Fixed: access first tbody
